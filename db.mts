...
export class SqliteMirrorRepository {
  ...
  deleteMessage(id: Model.Snowflake) {
    this._db
      .prepare(
        dedent`\
  ...
  deleteReaction(message_id: Model.Snowflake, author_id: Model.Snowflake, emoji: Model.Emoji) {
    this._db
      .prepare(
        dedent`\
          DELETE FROM message_reactions
